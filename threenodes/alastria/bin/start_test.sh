...
if [[ "$CARPETA" == "validator" ]]; then
	...
	sleep 15
	nohup env PRIVATE_CONFIG=~/alastria/"$CARPETA"/constellation/constellation.conf geth --datadir ~/alastria/"$CARPETA" --debug $GLOBAL_ARGS 2>> ~/alastria/logs/quorum_"$CARPETA"_"${_TIME}".log &
fi
